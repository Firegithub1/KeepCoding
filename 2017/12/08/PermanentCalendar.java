...
public class PermanentCalendar extends JFrame {
    ...
    private String updateLabel(int increment) {
        ...
        SimpleDateFormat format = new SimpleDateFormat("yyyy 年 MM 月"); // 设置字符串格式
        return format.format(calendar.getTime()); // 获得指定格式的字符串
    }
    ...
    private void updateTable(Calendar calendar) {
        ...
        int monthDay = calendar.getActualMaximum(Calendar.DAY_OF_MONTH); // 获得当前月的天数
        ...
        for (int i = 1; i < monthDay; i++) { // 遍历当前月的所有天并将其添加的二维数组中
