/**
 * 241：基于 Base64 编码（其他 Commons 组件简介）
 */
...
public class Base64EncodingFrame extends JFrame {
    ...
    protected void do_button_actionPerformed(ActionEvent e) {
        ...
        if (sourceString.length() == 0) { // 如果字符串长度为 0 则提示用户重新输入
            JOptionPane.showConfirmDialog(this, "请输入要编码的字符串", "", JOptionPane.WARNING_MESSAGE);
            return;
        }
        Base64 base64 = new Base64();
        byte[] encodeType = base64.encode(sourceString.getBytes()); // 进行编码
        message2TextArea.setText(new String(encodeType)); // 显示编码后的结果
    }
}
