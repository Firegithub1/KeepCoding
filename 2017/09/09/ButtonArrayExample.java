/**
 * 047：按钮控件数组实现计算器界面（JButton控件的二维数组）
 * <p>
 * 仅仅只是一个计算器界面，什么功能也没有。
 */
...
public class ButtonArrayExample extends JFrame {
    ...
    public ButtonArrayExample() {
        ...
        gridLayout.setHgap(5); // 设置组件的垂直间距

        contentPane = new JPanel(); // 获得容器对象
        contentPane.setLayout(gridLayout); // 设置容器采用网格布局管理器
        ...
        for (int row = 0; row < names.length; row++) {
            for (int col = 0; col < names.length; col++) {
                ...
                contentPane.add(buttons[row][col]); // 将按钮添加到面板中
            }
        }
    }
}
