...
public class ProgressMonitorTest extends JFrame {
    ...
    protected void do_button_actionPerformed(ActionEvent e) {
        ...
        monitor = new ProgressMonitor(ProgressMonitorTest.this, "正在计算素数", null, 0, max);
        new Timer(500, new CancelAction()).start(); // 利用计时器周期性地检查是否单击了“取消”按钮
    }
    ...
    private class CancelAction implements ActionListener {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            if (monitor.isCanceled()) { // 如果单击“取消”按钮则停止线程
                activity.cancel(true);
            } else if (activity.isDone()) { // 如果线程运行结束则停止进度监视器
                monitor.close();
                button.setEnabled(true);
            } else {
                monitor.setProgress(activity.getProgress()); // 更新进度监视器
            }
    ...
    private class Activity extends SwingWorker<Void, Integer> {
        ...
        @Override
        protected void process(List<Integer> chunks) { // 更新文本区和进度监视器
        ...
        private boolean isPrime(int number) { // 计算素数
            if (number < 2) {
                return false;
            } else {
