...
public class HighLightKeyWord extends JFrame {
    ...
    protected void do_button_actionPerformed(ActionEvent e) {
        ...
        String content = editorPane.getText(); // 获得 JEditorPane 中的所有文本
        Highlighter highlighter = editorPane.getHighlighter(); // 获得默认的 Highlighter 对象
        highlighter.removeAllHighlights(); // 移除原有的高亮显示区域
        if (content.contains(key)) { // 如果包含关键字
            int index = content.indexOf(key); // 确定第一个关键字的位置
            while (true) {
