...
public class EditableTree3 extends JFrame {
    ...
    public static void main(String[] args) {
        ...
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    EditableTree3 frame = new EditableTree3();
    ...
    protected void do_button_actionPerformed(ActionEvent e) {
        String key = textField.getText(); // 获得用户输入的关键字
        if ((key == null) || (key.isEmpty())) { // 如果关键字为空则提示用户重新输入
            JOptionPane.showMessageDialog(this, "请输入关键字", "", JOptionPane.WARNING_MESSAGE);
            return;
        }
        DefaultTreeModel model = (DefaultTreeModel) tree.getModel(); // 获得树模型
        DefaultMutableTreeNode targetNode = null;
        Enumeration enums = root.breadthFirstEnumeration(); // 获得树的全部节点
        while (enums.hasMoreElements()) { // 遍历全部节点进行查找
