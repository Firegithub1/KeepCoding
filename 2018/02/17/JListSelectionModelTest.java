...
public class JListSelectionModelTest extends JFrame {
    ...
    public JListSelectionModelTest() {
        ...
        selectAllButton.setFont(new Font("微软雅黑", Font.PLAIN, 14));
        ...
        selectNoneButton.setFont(new Font("微软雅黑", Font.PLAIN, 14));
    ...
    protected void do_selectAllButton_actionPerformed(ActionEvent e) {
        int start = 0;
        int end = list.getModel().getSize() - 1; // 获得最后一个列表项的索引
        if (end >= 0) { // 如果索引不小于0，则列表项个数至少为1
            list.setSelectionInterval(start, end); // 选择全部的列表项
        }
    }
