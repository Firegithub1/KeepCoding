...
public class LookAndFeelTest extends JFrame {
    ...
    public LookAndFeelTest() {
        ...
        UIManager.LookAndFeelInfo looks[] = UIManager.getInstalledLookAndFeels(); // 获得外观组
        for (UIManager.LookAndFeelInfo lookAndFeelInfo : looks) {
            comboBox.addItem(lookAndFeelInfo.getClassName()); // 为组合框增加选项
        }
    ...
    protected void do_comboBox_actionPerformed(ActionEvent e) {
        final String lookAndFeel = (String) comboBox.getSelectedItem(); // 获得用户的选项
        SwingUtilities.invokeLater(new Runnable() {

            @Override
            public void run() {
                try {
                    UIManager.setLookAndFeel(lookAndFeel); // 设置用户选择的外观
                } catch (Exception e) {
                    e.printStackTrace();
                }
                SwingUtilities.updateComponentTreeUI(contentPane); // 更新面板中的所有控件外观
            }
        });
    }
}
