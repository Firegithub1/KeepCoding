...
public class JPasswordFieldTest extends JFrame {
    ...
    protected void do_submitButton_actionPerformed(ActionEvent e) {
        ...
        if (password1.length < 6) { // 如果密码的长度小于 6 位则发出警告信息
            JOptionPane.showMessageDialog(this, "密码长度小于 6 位", "", JOptionPane.WARNING_MESSAGE);
        } else if (!Arrays.equals(password1, password2)) { // 如果密码的长度不同则发出警告信息
            JOptionPane.showMessageDialog(this, "两次密码不同", "", JOptionPane.WARNING_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(this, "两次密码相同", "", JOptionPane.WARNING_MESSAGE);
        }
    }
}
