...
public class ChinaGeographyTree extends JFrame {
    ...
    protected void do_this_windowActivated(WindowEvent e) {
        ...
        municipalities.add(new DefaultMutableTreeNode("北京")); // 为“直辖市”增加子节点“北京”
        ...
        root.add(municipalities); // 为根节点增加“直辖市”节点
        root.add(province); // 为根节点增加“省”节点
        root.add(ARegion); // 为根节点增加“自治区”节点
        root.add(SARegion); // 为根节点增加“特别行政区”节点
        DefaultTreeModel model = new DefaultTreeModel(root); // 利用根节点创建树模型
        tree.setModel(model); // 为树设置新的模型
    }
}
