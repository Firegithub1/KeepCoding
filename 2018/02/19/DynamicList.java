...
public class DynamicList extends JFrame {
    ...
    protected void do_addButton_actionPerformed(ActionEvent e) {
        String text = JOptionPane.showInputDialog("添加元素"); // 利用对话框获得用户输入的新列表项
        if ((text != null) && (!text.trim().isEmpty())) { // 判断列表项是否为空
            model.addElement(text.trim()); // 增加新列表项
        } else {
            return;
        }
    }
    ...
    protected void do_deleteButton_actionPerformed(ActionEvent e) {
        model.removeElement(list.getSelectedValue()); // 删除用户选择的列表项
    }
}
