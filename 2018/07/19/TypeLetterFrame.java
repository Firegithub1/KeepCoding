...
public class TypeLetterFrame extends JFrame {
    ...
    // 创建背景面板对象
    private final BackgroundPanel1 backgroundPanel1 = new BackgroundPanel1();
    ...
    public TypeLetterFrame() {
        ...
        backgroundPanel1.setImage(SwingResourceManager4.getImage(
                TypeLetterFrame.class, "/image/background.jpg"));
        getContentPane().add(backgroundPanel1, BorderLayout.CENTER);

        label.setBounds(10, 10, 157, 18);
        backgroundPanel1.add(label);

        lb_speed = new JLabel();
        lb_speed.setBounds(173, 10, 157, 18);
        backgroundPanel1.add(lb_speed);

        lb_ok = new JLabel();
        lb_ok.setBounds(345, 10, 157, 18);
        backgroundPanel1.add(lb_ok);
        addLetter();
    ...
    private void addLetter() {
        ...
        for (int i = 0; i < letters.length; i++) {
            ...
            // 设置标签显示的图片，即带字母的苹果
            labels[i].setIcon(SwingResourceManager4.getIcon(
            ...
            backgroundPanel1.add(labels[i]); // 将标签添加到背景面板中
            seed += 60; // 调整标签之间的偏移量
                    ...
                    if (s.equals(str)) { // 按键字符与向量中的相同
                        for (int j = 0; j < labels.length; j++) { // 遍历标签数组
                            if (str.equals(labels[j].getToolTipText())) { // 按键字符与标签提示文本相同
                                ...
                                // 设置标签上显示的字母苹果图片
                                labels[j].setIcon(SwingResourceManager4.getIcon(
